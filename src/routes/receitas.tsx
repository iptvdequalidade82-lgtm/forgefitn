...
export const Route = createFileRoute("/receitas")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Biblioteca FORGEFIT com centenas de receitas, cardápios e guias de alimentação.",
      },
      { property: "og:title", content: "Receitas, cardápios e guias — FORGEFIT" },
      {
        property: "og:description",
        content: "Encontre receitas completas e materiais para organizar sua alimentação.",
      },
...
function Receitas() {
  ...
  const lista = todas
    ...
    .filter(
      (r) =>
        !termo ||
        [r.nome, r.colecao, r.categoria, ...r.ingredientes].some((v) =>
  ...
  return (
    <div>
      <PageHeader
        titulo="Receitas"
        descricao="Receitas completas, cardápios e guias para cuidar da sua alimentação."
      />

      <Tabs defaultValue="receitas" className="space-y-5">
        <TabsList className="grid h-auto w-full grid-cols-2 p-1 sm:w-[420px]">
          <TabsTrigger value="receitas" className="min-h-10 gap-2">
            <UtensilsCrossed /> Receitas
          </TabsTrigger>
          <TabsTrigger value="guias" className="min-h-10 gap-2">
            <BookOpen /> Cardápios e guias
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="receitas" className="space-y-5">
          <section className="grid gap-3 sm:grid-cols-3" aria-label="Resumo da biblioteca">
            ...
            <Resumo
              numero={todas.filter((r) => r.calorias).length}
              rotulo="com calorias informadas"
            />
          </section>
          ...
          <section className="card-surface space-y-4 p-4" aria-label="Buscar e filtrar receitas">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Filtro
                ...
                value={colecao}
                onChange={setColecao}
                todos="Todas as coleções"
                opcoes={colecoesReceitas}
              />
              <Filtro
                label="Refeição"
                value={refeicao}
                onChange={setRefeicao}
                todos="Todas as refeições"
                opcoes={[...refeicoes]}
              />
            </div>
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-sm font-medium" aria-live="polite">
                {lista.length} {lista.length === 1 ? "receita encontrada" : "receitas encontradas"}
              </p>
          ...
          {lista.length === 0 ? (
            <EmptyState
              ...
              descricao="Tente outro nome, ingrediente ou limpe os filtros."
              ...
              <ul className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {lista.slice(0, visiveis).map((r) => (
                  <li key={r.id}>
                    <ReceitaCard
                      receita={r}
                      favorita={isFavorito("receitas", r.id)}
                      onFavoritar={() => toggleFavorito("receitas", r.id)}
        ...
        <TabsContent value="guias" className="space-y-5">
          <div className="border-l-4 border-primary pl-4">
            ...
            <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
              Use estes materiais como apoio educativo. Eles não substituem orientação individual de
          ...
          <ul className="grid gap-4 lg:grid-cols-3">
            {guiasAlimentacao.map((guia) => (
              <li key={guia.id} className="card-surface overflow-hidden">
                <Thumbnail
                  src={guia.capaUrl}
                  alt={`Capa de ${guia.titulo}`}
                  className="aspect-[16/9] w-full object-cover object-top"
                  fallbackIcone={<BookOpen className="h-8 w-8" />}
                />
                <div className="space-y-3 p-4">
                  <div className="flex flex-wrap gap-2">
                    <Chip className="bg-primary/15 text-primary">{guia.tipo}</Chip>
                    <Chip>{guia.paginas} páginas</Chip>
                  </div>
                  <div>
                  ...
                  <ul className="flex flex-wrap gap-1.5" aria-label="Assuntos do material">
                    {guia.topicos.map((t) => (
                      <li key={t}>
                        <Chip>{t}</Chip>
                      </li>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <Button onClick={() => window.open(guia.arquivoUrl, "_blank", "noopener")}>
                      <ExternalLink /> Ler guia
                    </Button>
                    <Button
                      variant="outline"
                      onClick={() => baixarArquivo(guia.arquivoUrl, guia.titulo)}
                    >
                      <Download /> Baixar
                    </Button>
                  </div>
      ...
      <Dialog open={Boolean(detalhe)} onOpenChange={(v) => !v && setDetalhe(null)}>
        <DialogContent className="max-h-[92vh] overflow-y-auto sm:max-w-2xl">
          {detalhe ? (
            <DetalheReceita
              receita={detalhe}
              favorita={isFavorito("receitas", detalhe.id)}
              onFavoritar={() => toggleFavorito("receitas", detalhe.id)}
              onAdicionar={() => setParaSemana(detalhe)}
            />
          ) : null}
        </DialogContent>
      </Dialog>
...
function Resumo({ numero, rotulo }: { numero: number; rotulo: string }) {
  return (
    <div className="card-surface flex items-center gap-3 p-4">
      ...
      <div>
        <strong className="font-display text-2xl leading-none">{numero}</strong>
    ...
    <article className="card-surface flex h-full min-h-52 flex-col p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <Button
          variant="ghost"
          size="icon"
          aria-label={favorita ? "Remover dos favoritos" : "Favoritar receita"}
          aria-pressed={favorita}
          onClick={onFavoritar}
        >
          <Heart className={cn(favorita && "fill-accent text-accent")} />
        </Button>
      </div>
      <h3 className="mt-3 font-display text-2xl font-semibold uppercase leading-tight">
        {receita.nome}
      </h3>
      <div className="mt-2 flex flex-wrap gap-1.5">
        <Chip>{receita.refeicao}</Chip>
        {receita.calorias ? <Chip>{receita.calorias} kcal</Chip> : null}
        {receita.rendimento ? <Chip>{receita.rendimento}</Chip> : null}
      </div>
      <p className="mt-3 text-sm text-muted-foreground">
        {receita.ingredientes.length} ingredientes • preparo completo
      </p>
      <Button className="mt-auto w-full" onClick={onAbrir}>
        Ver receita
      </Button>
    </article>
  );
...
function DetalheReceita({
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle className="pr-8 text-left font-display text-3xl uppercase leading-tight">
          {receita.nome}
        </DialogTitle>
      </DialogHeader>
      ...
      <section>
        <h3 className="mb-3 font-display text-xl font-semibold uppercase">Ingredientes</h3>
        <ul className="space-y-2">
          {receita.ingredientes.map((i, n) => (
            <li key={`${n}-${i}`} className="flex gap-2 text-sm leading-relaxed">
      ...
      <section>
        <h3 className="mb-3 font-display text-xl font-semibold uppercase">Modo de preparo</h3>
        <ol className="space-y-3">
          {receita.modoPreparo.map((p, i) => (
            <li key={i} className="grid grid-cols-[28px_1fr] gap-2 text-sm leading-relaxed">
              <span className="grid h-7 w-7 place-items-center rounded-md bg-primary/15 font-semibold text-primary">
                {i + 1}
              </span>
              <span>{p}</span>
            </li>
      ...
      <div className="grid gap-2 sm:grid-cols-2">
        <Button onClick={onAdicionar}>
          <CalendarPlus /> Adicionar à semana
        </Button>
        <Button variant="outline" onClick={onFavoritar}>
          <Heart className={cn(favorita && "fill-accent text-accent")} />
          {favorita ? "Salva nos favoritos" : "Salvar receita"}
        </Button>
        <Button
          variant="outline"
          className="sm:col-span-2"
          onClick={() => window.open(receita.arquivoDownloadUrl, "_blank", "noopener")}
        >
          <BookOpen /> Abrir livro original
        </Button>
      </div>
    </>
  );
...
function EscolherDiaRefeicao({
  ...
  const [ref, setRef] = React.useState(
    refeicaoInicial && refeicoes.includes(refeicaoInicial as (typeof refeicoes)[number])
      ? refeicaoInicial
      : refeicoes[0],
  );
  return (
    <div className="space-y-4">
      ...
      <Button className="h-11 w-full" onClick={() => onConfirmar(Number(dia), ref)}>
        Salvar na semana
      </Button>
    </div>
  );
