...
function EscolherDiaRefeicao({ refeicaoInicial, onConfirmar }: { refeicaoInicial?: string; onConfirmar: (dia: number, refeicao: string) => void }) {
  ...
  return <div className="space-y-4"><div><Label className="mb-1.5 block text-xs font-medium uppercase text-muted-foreground">Dia</Label><Select value={dia} onValueChange={setDia}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent>{DIAS.map((d, i) => <SelectItem key={d} value={String(i + 1)}>{d}</SelectItem>)}</SelectContent></Select></div><div><Label className="mb-1.5 block text-xs font-medium uppercase text-muted-foreground">Refeição</Label><Select value={ref} onValueChange={setRef}><SelectTrigger className="h-11"><SelectValue /></SelectTrigger><SelectContent>{refeicoes.map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}</SelectContent></Select></div><Button className="h-11 w-full" onClick={() => onConfirmar(Number(dia), ref)}>Salvar na semana</Button></div>;
}